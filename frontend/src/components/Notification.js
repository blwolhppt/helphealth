...
const NotificationSidebar = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await fetch('http://81.26.183.116:8000/api/notifications/', {
        headers: {
  ...
  const markAsRead = async (id) => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`http://81.26.183.116:8000/api/notifications/${id}/`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const markAllRead = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch('http://81.26.183.116:8000/api/notifications/mark_all_read/', {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
